...
public class ProductProc implements ProductProcInter {
  ...
  public List<ProductVO> list_all(HashMap<String, Object> map) {
    ...
    List<ProductVO> list = this.productDAO.list_all(map);
  ...
  public int search_all_count(HashMap<String, Object> hashMap) {
    int count = productDAO.search_all_count(hashMap);
    return count;
  }  
  ...
  public String pagingBox(String listFile, int search_count, int nowPage, String searchword) {
    ...
    int _nowPage = (nowGrp-1) * Product.PAGE_PER_BLOCK;  
    if (nowGrp >= 2){ 
      str.append("<span class='span_box_1'><A href='"+listFile+"?searchword="+searchword+"&nowPage="+_nowPage+"'>이전</A></span>"); 
    } 
    ...
    for(int i=startPage; i<=endPage; i++){ 
      ...
      if (nowPage == i){ // 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
        ...
        // 현재 페이지가 아닌 페이지는 이동이 가능하도록 링크를 설정
        str.append("<span class='span_box_1'><A href='"+listFile+"?searchword="+searchword+"&nowPage="+i+"'>"+i+"</A></span>");   
      } 
    } 
 
    _nowPage = (nowGrp * Product.PAGE_PER_BLOCK)+1;  
    if (nowGrp < totalGrp){ 
      str.append("<span class='span_box_1'><A href='"+listFile+"?searchword="+searchword+"&nowPage="+_nowPage+"'>다음</A></span>"); 
    } 
    str.append("</DIV>"); 
