...
<jsp:include page="/menu/side.jsp" />
  ...
  <DIV id='panel_create' style='padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    <FORM name='frm_create' id='frm_create' method='POST' 
      ...
      <label>그룹 이름</label>
      <input type='text' name='name' value='${recipecategrpVO.name }' required="required" style='width: 25%;'>
 
      <label>출력 순서</label>
      <input type='number' name='seqno' value='${recipecategrpVO.seqno }' required="required" 
                min='1' max='1000' step='1' style='width: 5%;'>
  
      <label>출력 형식</label>
      <select name='visible'>
        <option value='Y' ${recipecategrpVO.visible == 'Y' ? "selected='selected'" : "" }>Y</option>
        <option value='N' ${recipecategrpVO.visible == 'N' ? "selected='selected'" : "" }>N</option>
      </select>
